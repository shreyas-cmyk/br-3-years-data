...
# ---------------- STEP 5: SERVICE COUNTS ----------------
service_counts = df.groupby("year")[service_columns].sum().reset_index()

# ---------------- STEP 6: CALCULATE PERCENTAGES ----------------
result = service_counts.merge(total_centers, on="year")

for col in service_columns:
    result[col] = round(
        (result[col] / result["total_centers"]) * 100,
        1
    )

# ---------------- STEP 7: FINAL OUTPUT ----------------
final_percentage_table = result[["year"] + service_columns]

print("\n===== USE CASE 2: YEAR-WISE % SERVICES EXPANSION =====")
