...
sheet = client.open("CM Data new copy").worksheet("CM-2025")
...
# Reorder columns exactly as DB
df = df[expected_columns]

# Replace empty strings with NULL
df = df.replace({"": None})

# ---------------- STEP 5: POSTGRES CONNECTION (Neon-safe) ----------------
conn_string = (
     "Enter your DB URL"
)
...
CHUNK_SIZE = 200   # 🔥 Safe for Neon
total_rows = len(df)

with engine.begin() as conn:
    conn.execute(text("TRUNCATE TABLE public.centers_consolidated"))
    print("🧹 centers_consolidated table truncated")
...
print("🎉 Database table 'centers_consolidated' successfully refreshed from Google Sheet.")
